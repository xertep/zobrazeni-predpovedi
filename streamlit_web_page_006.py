    ...
    for pattern, label in MOUNTAIN_FORECAST_TYPES:
        ...
        try:
            ...
            for item in items:
                ...
                t = item.get("displayText")
                if h:
                    output_lines.append(f"### {h}")  # bigger header

                if t:
                    t = t.replace("\xa0", " ")
                    output_lines.append(t)
                    output_lines.append("")  # space after paragraph
...
/* JM */
button[kind="secondary"][data-testid*="region_JM"] {
    background-color: pink;
}

/* ZL */
button[data-testid*="region_ZL"] {
    background-color: palegreen;
}

/* VY */
button[data-testid*="region_VY"] {
    background-color: skyblue;
}

/* CR */
button[data-testid*="region_CR"] {
...
def button_grid(items, cols_per_row, key_prefix):
    ...
    for row in rows:
        ...
        for i, code in enumerate(row):
            if cols[i].button(code, key=f"{key_prefix}_{code}"):
                selected = code
    return selected

# Regions buttons
st.markdown("### Kraje")
selected_region = button_grid(region_codes, 6, "region")

# Mountains buttons
mountain_codes = [code for code, _ in mountains]
selected_mountain = button_grid(mountain_codes, 5, "mountain")
