...
region = st.selectbox(
    "Vyber kraj:",
    ["CR", "JM", "ZL", "VY", "CB", "HK", "KV", "LB", "MS", "OL", "PH", "PL", "PU", "SC", "UL"]
)

mountain = st.selectbox(
    "Horské oblasti:",
    [code for code, _ in mountains] + ["None"]
)

if st.button("Zobrazit předpověď"):
    if mountain != "None":
        st.markdown(fetch_mountain(mountain))
    else:
        st.markdown(fetch_region(region))
